...
const ManageComparisonWizard = () => {
    ...
    const handleEditComparison = async (researchProblem, lastDirectory, properties, comparisonId) => {
        ...
        try {
            const response = await axios.post(`${env('COMPARISON_WIZARD_API')}edit-comparison`, dataToSend);
    ...
    const handleAddComparison = async (researchProblem, lastDirectory) => {
        ...
        try {
            const response = await axios.post(`${env('COMPARISON_WIZARD_API')}add-comparison`, dataToSend);
    ...
    const DraggableResizableDiv = ({ children, initialWidth, initialHeight, left, top, headText, customText, tapTopPosistion, tapLeftPosition }) => {
        ...
        return (
            <>
                {!isCollapsed && (
                    <Draggable>
                        <Resizable width={size.width} height={size.height} minConstraints={[80, 80]} onResize={handleResize} resizeHandles={['se']}>
                            <div
                                style={{
                                    position: 'absolute',
                                    top,
