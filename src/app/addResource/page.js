...
const AddResource = () => {
    ...
    useEffect(() => {
        ...
        const getDefaultClass = async () => {
            let classes = searchParams.get('classes')?.split(',');
            if (!isCurationAllowed && classes && classes.length > 0) {
        ...
        if (label.trim() !== '') {
            if (!isDOI.test(label)) {
                try {
                    ...
                    setIsLoading(false);
                    router.push(`${reverse(ROUTES.RESOURCE, { id: newResource.id })}?noRedirect`);
                } catch (error) {
                    console.error(error);
                    setIsLoading(false);
