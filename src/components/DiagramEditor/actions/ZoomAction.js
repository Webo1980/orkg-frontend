    ...
    constructor() {
        super({
            ...
            fire: ({ event }) => {
                ...
                const currentZoomLevelIndex = ZOOM_LEVELS.indexOf(ZOOM_LEVELS.includes(model.getZoomLevel()) ? model.getZoomLevel() : 100);

                let nextZoomLevelIndex;
                if (zoomDirection === 'in') {
                    nextZoomLevelIndex = Math.min(currentZoomLevelIndex + 1, ZOOM_LEVELS.length - 1);
                } else {
                ...
                const boundingRect = this.engine.getCanvas().getBoundingClientRect();
