...
function Sidebar(props) {
    ...
    return (
        <StyledSideBar className="col-md-3">
            <SideBarHeaderStyle>Resources from abstract </SideBarHeaderStyle>
            <div className="d-block">
                <div className="p-2">
                    ...
                    {!props.isAbstractLoading && props.isAnnotationLoading && (
                        <ContentLoader title={'Loading annotation'} height={120} speed={2} primaryColor="#f3f3f3" secondaryColor="#ecebeb">
                            <rect x="0" y="18" rx="7" ry="7" width="340" height="120" />
                        </ContentLoader>
                    )}
                    {!props.isAnnotationLoading && props.isAnnotationFailedLoading && <div>Failed to connect to the annotation service!</div>}
                    {!props.isAbstractLoading && !props.isAbstractFailedLoading && !props.isAnnotationLoading && !props.isAnnotationFailedLoading && (
                        <div className="p-2">
                            <StyledGraggableData>
                                {toArray(props.ranges)
                                    ...
                                    .slice(0, rangesLimit)
                                    .map(range => {
                                        return <DraggableValue key={`s${range.id}`} resourceId={range.resourceId} id={range.id} label={range.text} />;
                                    })}
                            </StyledGraggableData>
