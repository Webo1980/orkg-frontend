...
const nextLinkPosition = (event, model, initialRelative, sourcePosition) => {
    ...
    return snap(new Point(linkNextX, linkNextY), model.options.gridSize);
};

/**
 * Handles mouse move on link or creating and
...
export default function handleLinkDrag(event, link) {
    ...
    if (this.hasStartedMoving) {
        if (!link.hasMiddlePoint()) {
            ...
            if (samePosition(middle, last)) {
                ...
                link.removePoint(link.getMiddlePoint());
                this.moveDirection = this.moveDirection === 'horizontal' ? 'vertical' : 'horizontal';
     ...
     */
    if (!link.hasMiddlePoint() && first.x !== next.x && first.y !== next.y) {
