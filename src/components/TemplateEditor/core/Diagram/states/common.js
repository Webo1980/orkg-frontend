...
export const snap = (position, gridSize = 15) => {
    if (position instanceof Point) {
        return new Point(Math.round(position.x / gridSize) * gridSize, Math.round(position.y / gridSize) * gridSize);
    }
 ...
 */
export const sameAxis = (...points) => sameX(...points) || sameY(...points);
...
export const closestPointToLink = (P, link) => {
    ...
    const closestPointOnSegment = (Q, segment) => {
        ...
        if (t >= 0 && t <= 1) {
            const closest = new Point((1 - t) * A.x + t * B.x, (1 - t) * A.y + t * B.y);
            return {
                point: closest,
        ...
        return distanceToA <= distanceToB ? { point: A, distance: distanceToA } : { point: B, distance: distanceToB };
    };
 ...
 */
export const isPointOverLink = (point, link) => isPointOverFirstLinkSegment(point, link) || isPointOverSecondLinkSegment(point, link);
