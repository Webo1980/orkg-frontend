...
import LinkModel from 'components/TemplateEditor/core/Link/LinkModel';
...
export default class DeleteAction extends Action {
    ...
    fireEvent = entities => {
        // All selected nodes
        const nodes = entities.filter(model => model instanceof BaseModel);

        // All selected links
        const links = entities.filter(model => model instanceof LinkModel);

        // All links from selected nodes
        const nodesLinks = nodes.reduce((arr, node) => [...arr, ...node.getAllLinks()], []);

        this.engine.fireEvent({ nodes, links: [...nodesLinks, ...links] }, 'entitiesRemoved');
    };
}
