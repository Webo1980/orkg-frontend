...
export default class ClipboardAction extends Action {
    ...
    matchesInput = ({ ctrlKey, code }) => ctrlKey && (code === 'KeyX' || code === 'KeyC' || code === 'KeyV');
    ...
    handleCut = () => {
        ...
        this.engine.fireEvent(
            {
                nodes: selected,
                links: selected.reduce((arr, node) => [...arr, ...node.getAllLinks()], [])
            },
            'entitiesRemoved'
        );
    ...
    /** Copy */
    handleCopy = () => {
        const copies = this.getSelectedComponents().map(entity => entity.clone().serialize());
