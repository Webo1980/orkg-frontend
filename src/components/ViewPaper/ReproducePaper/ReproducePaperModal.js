    ...
    if (props.showReproducePaperModalDialog === true) {
            ...
            if (propertyValue != '') {
                if (propertyType == 'literal' && propertyValue.startsWith('http')) {
                    ...
                    if (checkedPropertyAccessibility == true) {
                        ...
                        accessibility.push(
                            <Tippy content={<>The given url: {propertyValue} is reachable</>}>
                                <Icon icon={faCheck} className="mt-2 text-success" />
                            </Tippy>,
                        );
                    } else {
                        accessibilityScore[propertyValue] = 0;
                        accessibility.push(
                            <Tippy content={<>The given url: {propertyValue} is unreachable</>}>
                                <Icon icon={faXmark} className="mt-2 text-danger" />
                            </Tippy>,
                        );
                    }
                } else {
                    accessibilityScore[propertyValue] = 100;
                    accessibility.push(
                        <Tippy content={<>The property type : {propertyType} is not a valid url, so it is inapplicable for this check</>}>
                ...
                accessibilityScore[propertyValue] = 0;
                accessibility.push(
                    <Tippy content="The property has to have at least one value">
                        <Icon icon={faTimes} className="mt-2 text-danger" />
                    </Tippy>,
                );
            }
        ...
        let checkAvailability = propertyValue => {
            ...
            if (propertyValue != '') {
                ...
                availability.push(
                    <Tippy content={<>The property has value: {propertyValue}</>}>
                        <Icon icon={faCheck} className="mt-2 text-success" />
                    </Tippy>,
                );
            } else {
            ...
            if (propertyType == 'resource') {
                ...
                if (sameAsValueId) {
                    ...
                    if (valueID == '') {
                        ...
                        linkability.push(
                            <Tippy content={<>The property is not of type resource, and cannot be linked with other resources</>}>
                                <Icon icon={faXmark} className="mt-2 text-danger" />
                            </Tippy>,
                        );
                    } else {
                    ...
                    linkabilityScore[propertyValue] = 0;
                    linkability.push(
                        <Tippy content={<>The resource {propertyValue} is not linked to any external ontolgies</>}>
                            <Icon icon={faXmark} className="mt-2 text-danger" />
                        </Tippy>,
                    );
                }
                ...
                linkabilityScore[propertyValue] = 100;
                linkability.push(
                    <Tippy content={<>The property is not of type resource, so it is inapplicable for this check</>}>
                        <Icon icon={faMinus} style={{ color: '#D4D4D4', fontWeight: 'bolder', fontSize: '1.5em' }} />
                    </Tippy>,
                );
            }
                ...
                if (propertyType == 'literal' && propertyValue.startsWith('http')) {
                    ...
                    if (domain == 'github') {
                        ...
                        if (checkedPropertyLicense != '') {
                            ...
                            license.push(
                                <Tippy content={<>The license of the given url: {propertyValue} cannot be retrived</>}>
                                    <Icon icon={faXmark} className="mt-2 text-danger" />
                                </Tippy>,
                            );
                        }
                        ...
                        licenseScore[propertyValue] = 100;
                        license.push(
                            <Tippy content={<>The license of the given url cannot be checked. For now we check only license for Github repos</>}>
                                <Icon icon={faCheck} className="mt-2 text-success" />
                            </Tippy>,
                        );
                    }
        ...
        if (requiredPropertiesIDsForReproducibility.length > 0) {
            ...
            Object.entries(requiredPropertiesIDsForReproducibility).map((key, index) => {
                ...
                Object.entries(itemValues).map((propertyValueKey, propertyValueIndex) => {
                    ...
                    const totalPropertyScore =
                        (accessibilityData[1][propertyValue] +
                            availabilityData[1][propertyValue] +
                            linkabilityData[1][propertyValue] +
                            licenseData[1][propertyValue]) /
                        4;
        ...
        const ReproducibilityTable = SortableContainer(() => {
            if (isReproducibilityTemplateUsed.includes(true)) {
                        ...
                        <TableContainerStyled>
                            <Table bordered hover className="m-0">
                                <thead>
                                    <tr>
                                        ...
                                        <td key="3" style={{ width: '19%', fontWeight: 'bold' }}>
                                            Availability{' '}
                                            <Tippy content="The availability means that the given resource has at least a one value">
                                                <Icon icon={faQuestionCircle} />
                                            </Tippy>
                                        </td>
                                        <td key="4" style={{ width: '19%', fontWeight: 'bold' }}>
                                            Accessibility
                                            <Tippy content="The accessibility means that if the given resource of type URL, then the URL should be reachable">
                                                <Icon icon={faQuestionCircle} />
                                            </Tippy>
                                        </td>
                                        <td key="5" style={{ width: '17%', fontWeight: 'bold' }}>
                                            Linkability
                                            <Tippy content="The linkability means that the given resource is linked with an external ontology">
                                                <Icon icon={faQuestionCircle} />
                                            </Tippy>
                                        </td>
                                        <td key="6" style={{ width: '17%', fontWeight: 'bold' }}>
                                            Licence
                                            <Tippy content="The license means that if the given resource is of type URL (eg.,gitHub), then it should have a valid license">
                                                <Icon icon={faQuestionCircle} />
                                            </Tippy>
                                        </td>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td key="1" colSpan={2} style={{ width: '15%', fontWeight: 'bold', textAlign: 'center' }}>
                                            Final Score
                                        </td>
                ...
                return (
                    <>
                        <Alert color="danger">
                            At least you need to use one of the reproducibility templates. A reproducibility template, is a template that starts with
                            the key word "Reproducible"
                        </Alert>
                    </>
                );
            }
        ...
        // important to avoid errors at the first load time
