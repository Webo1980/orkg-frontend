...
const DisplaySimilarPapers = ({ similarPapers = [] }) => {
    return (
        <>
            {similarPapers.length > 0 && (
                <CardsContainer>
                    {similarPapers.map((contribution, index) => {
            ...
            {similarPapers.length === 0 && <Alert color="light">We couldn't find any similar contribution, please try again later</Alert>}
        </>
    );
};

DisplaySimilarPapers.propTypes = {
    similarPapers: PropTypes.array.isRequired
};

export default DisplaySimilarPapers;
