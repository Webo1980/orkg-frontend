import $ from 'jquery';
import workJSON from '../entities/work';
import personJSON from '../entities/work';
import customizeJSON from '../schemas/customize';
import indicatorsJSON from '../schemas/indicators';
 ...
 * @param identifier
 */
export function ImpactViz(identifier, options = '') {
    ...
    if (options.customize) {
        window.customizePath = options.customize;
    }
    if (options.indicators) {
        window.indicatorsPath = options.indicators;
    }
    ...
    this.initViz = function() {
        ...
        $('#impactviz-overview')
            .html('')
            .append(
                '<div id="impactviz-loading">Collecting the data for the entered ' + entity + '. Please be patient - this may take a while.</div>'
            );
        $('#impactviz-loading')
            .html('')
            .append('<img src="/ROSI/loading.gif"></img>');

        $('#impactviz-details').html('');
        // get schema for this entity type
        const schema = entity === 'work' ? workJSON : personJSON;
        ...
        $.getJSON(schema['api'] + identifier, function(json) {
            ...
            switch (entity) {
                ...
                case 'work':
                    ...
                    const conceptIds = [];
                    for (const key in schema['concepts']) {
                        conceptIds.push(key);
                    }
                    ...
                    $.each(conceptIds, function(index, conceptId) {
                        ...
                        displayPaperbuzzviz(
                            convertPaperbuzzData(json, schema['concepts'][conceptId]['sources'], conceptId),
                            '#' + conceptId + '-results'
                        );
                    });
            ...
            const schemaId = params.get('schema') || '0';

            // get the data for this entity by identifier
            getData(customizeJSON[schemaId]['indicators'], identifier, function(results) {
...
function getIndicatorById(indicatorId, callback) {
    ...
    $.each(indicatorsJSON, function(index, indicator) {
...
function displaySearchForm(identifier) {
    $('#form').alpaca({
        ...
        options: {
            label: 'Identifier',
            helper:
                'Visualize scientometric indicators: Enter persistent identitifiers (currently Digital Object Identifiers - DOI) or select one of the random examples of publications and research data below.',
            form: {
                buttons: {
                    view: {
                        label: 'Visualize!',
...
function displayConceptStructure(concept) {
    ...
    $('#impactviz-overview-row').append(
        '<div class="col-sm-3"><a id="' +
            concept.id +
            '-link" href="#"><img width="80px" src="/ROSI/' +
            concept.id +
            '-white.png" id="' +
            concept.id +
            '-image"></img><br/><h4>' +
    ...
    // display
    $('#impactviz-details').append(
        '<div class="section concept" id="' +
            concept.id +
...
function displayCustomizeForm() {
    ...
    $.each(customizeJSON, function(index, element) {
...
function writeData(concept, label, data, overview = false) {
    ...
    $('#' + concept + '-image').attr('src', '/ROSI/' + concept + '.png');

    // display icon if the data is binary
    if (data === true || data === false || data === null) {
        data = '<img width="20px" src="/ROSI/' + data + '.png"></img>';
    }
    ...
    if (overview && $('#paperbuzz-count-' + id + '-overview').length === 0) {
        $('#' + concept + '-overview').append(
            '<div class="paperbuzz-source-row paperbuzz-compact"><div class="paperbuzz-source-heading">' +
                label +
                '<div class="paperbuzz-count-label" id="paperbuzz-count-' +
                id +
                '-overview">' +
                data +
                '</div></div></div>'
...
function convertPaperbuzzData(json, sources, concept = '') {
    ...
    for (const object of data.altmetrics_sources) {
        let toss = true;
        ...
        if (toss) {
            delete data.altmetrics_sources[i];
        }
        i++;
    }
