...
function MaturityMinimumNeededReviews(props) {
    ...
    const { metaData, data } = useComparison({ id });
    const { votesCount } = ManageMaturityReviews({ data, metaData });
    ...
    const MaturityTable = SortableContainer(({ items }) => (
        <>
            <Alert color="info">
                ...
                - In your opinion, what are the minimum needed reviews to judge a comparison? For example, if you have decided to say 5 reviewers is
                the minimum number of reviews. This means less than 5 reviews for one comparison would not be sufficient to make the comparison move
                to the next level of maturity.
                <br />- After you submit your choice, the system will automatically select the most selected value to be the minimum needed number of
                reviews
            </Alert>
            <br />
            <TableContainerStyled>
                <Form onSubmit={e => handleSubmit(e)}>
                    <Table bordered hover className="m-0">
                        <thead>
                            <tr>
                                <td key="1">What is the minimum reviews?</td>
                                <td key="2">
                                    <select className="form-control-sm" id="reviewsCount" onChange={e => showHideOtherDiv(e)}>
                                    ...
                                    <span id="otherReviewsCountDiv" style={{ display: 'none' }}>
                                        <input
                                            ...
                                            onBlur={e =>
                                                isNaN(e.target.value) || e.target.value < 6 || e.target.value > 9
                                                    ? (e.target.value = 'Please type a number above 5 and less than 9')
                                                    : e.target.value
                                            }
                                        />
                                    </span>
