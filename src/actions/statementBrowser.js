...
// SOME ACTIONS for the graph explorationHandling

export const graphLoadContributionResource = ({ contributionId: id, contributionIsLoaded, contributionLabel }) => dispatch => {
    // we assume that the exploration switch does the correct thing (i.e., this is called only when contribution is not loaded
    if (!contributionIsLoaded) {
        dispatch({
            type: type.STATEMENT_BROWSER_ADD_CONTRIBUTION_OBJECT,
            payload: {
                id
            }
        });
        ...
        return Promise.resolve(getDataPromised(id, dispatch));
    }
};

export const loadMultipleResource = itemsArray => async dispatch => {
    console.log('SHOULD LOAD THE DATA');

    console.log('loadMultipleResource', itemsArray);
    for (let i = 0; i < itemsArray.length; i++) {
        console.log('EXECUTING PROMISED CALL ', i);
        await executePromisedItemLoad(itemsArray[i], dispatch);
    }
};
...
const executePromisedItemLoad = async (item, dispatch) => {
    ...
    if (!item.contributionOriginId) {
        console.log('FETCHING CONTRIBUTION!_', id);
        dispatch({
            type: type.STATEMENT_BROWSER_ADD_CONTRIBUTION_OBJECT,
            payload: {
                id
    ...
    await getDataPromised(id, dispatch);
    console.log('>>>> DONE ');
};

export const graphLoadResource = id => dispatch => {
    // we assume that the exploration switch does the correct thing (i.e., this is called only when contribution is not loaded
    // dispatch(
    //     createResource({
    //         //only needed for connecting properties, label is not shown
...
export const promisedFetchStatementsForResource = async (data, dispatch) => {
    ...
    const queryPromise = new Promise(function(resolve) {
        ...
        //TODO: make depth iterations from the data.depth parameter;
